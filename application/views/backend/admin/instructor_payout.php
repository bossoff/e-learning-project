                                        ...
                                        <tr class="gradeU">
                                            ...
                                            <td style="text-align: center;">
                                              <?php if ($pending_payout['status'] == 0): ?>
                                                <?php if ($paypal_keys['production_client_id'] != ""): ?>
                                                  <form action="<?php echo site_url('admin/paypal_checkout_for_instructor_revenue'); ?>" method="post">
                                                    ...
                                                    <input type="hidden" name="production_client_id" value="<?php echo $paypal_keys['production_client_id']; ?>">
                                                    <input type="submit" class="btn btn-outline-info btn-sm btn-rounded mb-1"        value="<?php echo get_phrase('paypal'); ?>">
                                                  </form>
                                                <?php else: ?>
                                                  <button type="button" class = "btn btn-outline-danger btn-sm btn-rounded mb-1" name="button" onclick="alert('<?php echo get_phrase('this_instructor_has_not_provided_valid_paypal_client_id'); ?>')"><?php echo get_phrase('paypal'); ?></button>
                                                <?php endif; ?>
                                                <?php if ($stripe_keys['public_live_key'] != "" && $stripe_keys['secret_live_key']): ?>
                                                  <div id="buynow-<?php echo $pending_payout['id']; ?>" style="height: 45px;">
                                                        <button class="stripe-button btn btn-outline-info btn-sm btn-rounded" id="stripePayButton-<?php echo $pending_payout['id']; ?>" onclick="stripe_checkout('<?php echo $stripe_keys['public_live_key']; ?>', '<?php echo $pending_payout['id']; ?>')"><?php echo get_phrase("stripe"); ?></button>
                                                    </div>
                                                <?php else: ?>
                                                  <button type="button" class = "btn btn-outline-danger btn-sm btn-rounded" name="button" onclick="alert('<?php echo get_phrase('this_instructor_has_not_provided_valid_public_key_or_secret_key'); ?>')"><?php echo get_phrase('stripe'); ?></button>
                                                <?php endif; ?>

                                                <?php if ($razorpay_keys['key_id'] != "" && $razorpay_keys['secret_key']): ?>
                                                    <form action="<?php echo site_url('admin/razorpay_checkout_for_instructor_revenue/'.$pending_payout['user_id'].'/'.$pending_payout['id']); ?>" method="post" class="razorpay-form form">
                                                        <input type="hidden" name="total_price_of_checking_out" value="<?php echo $pending_payout['amount']; ?>">
                                                        <button type="submit" class="btn btn-outline-info btn-sm btn-rounded"><?php echo get_phrase('razorpay'); ?></button>
                                                    </form>
                                                <?php else: ?>
                                                  <button type="button" class = "btn btn-outline-danger btn-sm btn-rounded" name="button" onclick="alert('<?php echo get_phrase('this_instructor_has_not_provided_valid_key_id_or_secret_key'); ?>')"><?php echo get_phrase('razorpay'); ?></button>
                                                <?php endif; ?>
